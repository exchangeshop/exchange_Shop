...
<body>
  <div class="ui text container" style ="text-align: center; background-color: #1eaef4; padding: 2% ">
     <h1> Trust to buy your Pre-Owned SSD</h1>
  ...
  <div id="searchbar" style="display: none; text-align: left;">
    <form class="ui form" method="get" action="used_ssd_searchresult.php">
      <div class="field">
        <label>Brand Name</label>
        <input type="text" name="keyword" placeholder="Partial SSD Brand Name">
      </div>
...
<?php
    ...
    $query = "SELECT * FROM `ssd` WHERE SSD_CATAGORY =0";
      ...
      $index = 1;
        ...
        while ($row = $result->fetch_assoc()) {
            printf ('<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td><a href="../buynow.php?">BUY</a> </td> 
              </tr>', $index, $row["SSD_NAME"], $row["SSD_PRICE"], $row["SSD_STORAGE"]);
