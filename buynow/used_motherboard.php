...
<html>
<head>
  <title>Preowned Motherboard</title>
  <meta charset="UTF-8">
...
<body>
  <div class="ui text container" style ="text-align: center; background-color: #1eaef4; padding: 2% ">
     <h1> Trust to buy your Pre-Owned Motherboard</h1>
  ...
  <div id="searchbar" style="display: none; text-align: left;">
    <form class="ui form" method="get" action="used_motherboard_searchresult.php">
      <div class="field">
        <label>Brand Name</label>
        <input type="text" name="keyword" placeholder="Partial Motherboard Brand Name">
      </div>
...
<?php
    ...
    $query = "SELECT * FROM `motherboard` WHERE MTH_CATAGORY= 0";
    ...
    if ($result = $conn->query($query)) {
      ...
      $index = 1;
        ...
        while ($row = $result->fetch_assoc()) {
            printf ('<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td><a href="../buynow.php?">BUY</a> </td> 
              </tr>', $index, $row["MTH_NAME"], $row["MTH_PRICE"], $row["GENERATION"], $row["MEMORY_TYPE"], $row["MEMORY_SLOT"],$row["MEMORY_MAX"]);
