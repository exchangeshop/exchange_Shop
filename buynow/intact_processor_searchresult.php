...
<body>
  ...
  <?php
   ...
   $query = "SELECT * FROM `processor` WHERE P_CATAGORY =1 AND P_NAME LIKE '%$keyword%'";
    
    if ($result = $conn->query($query)) {
      printf('<div class="ui text container">');
      printf('<table class="ui unstackable table">');
      printf("<thead> <tr> <th>Index</th> <th>Brand</th> <th>Price</th> <th>Cache Size</th> <th>Core</th> <th>Generation</th> <th>add</th> </tr> </thead>");

      $index = 1;
        /* fetch associative array */
        while ($row = $result->fetch_assoc()) {
            printf ('<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td><a href="../buynow.php?">BUY</a> </td> 
              </tr>', $index, $row["P_NAME"], $row["P_PRICE"], $row["CACHE_SIZE"], $row["CORE"], $row["P_TYPE"]);
