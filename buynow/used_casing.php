...
<body>
  <div class="ui text container" style ="text-align: center; background-color: #1eaef4; padding: 2% ">
     <h1> Trust to buy your Pre-Owned Casing</h1>
  ...
  <div id="searchbar" style="display: none; text-align: left;">
    <form class="ui form" method="get" action="used_casing_searchresult.php">
      <div class="field">
...
<?php
    ...
    $query = "SELECT * FROM `casing` WHERE CASING_CATAGORY = 0";
      ...
      $index = 1;
        ...
        while ($row = $result->fetch_assoc()) {
            printf ('<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td><a href="../buynow.php?">BUY</a> </td> 
              </tr>', $index, $row["CASING_NAME"], $row["CASING_PRICE"], $row["CASE_TYPE"], $row["SUPPORTED_MAINBOARD_TYPE"], $row["POWER_SUPPLY_BUILTIN"]);
